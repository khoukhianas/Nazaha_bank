...
class SignUpView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Ajout des logs pour chaque champ reçu
            logger.debug(f"Full name: {full_name}")
            logger.debug(f"Email: {email}")
            logger.debug(f"CINE: {cine}")
            logger.debug(f"Date of Birth: {date_of_birth}")
            logger.debug(f"Place of Birth: {place_of_birth}")
            logger.debug(f"ID card: {id_card}")
